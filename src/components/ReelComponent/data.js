export const VIDEO_DATA = [
  {
    id: 'V_ID_1',
    title: 'Iron Man',
    thumbnailUrl:
      'https://ew.com/thmb/OEm1NLRTUG5HuHswbWLjvLJ5GIg=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/iron-man-2000-c36c45429d5148e5a5871a887cc3d96c.jpg',
    duration: '8:18',
    uploadTime: 'May 9, 2011',
    views: '24.9M',
    author: 'Vlc Media Player',
    videoUrl:
      'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4',
    audioUrl:
      'https://github.com/rafaelreis-hotmart/Audio-Sample-files/raw/master/sample.mp3',
    description:
      'Tony Stark, a billionaire genius, dons his iconic red-and-gold suit to fight against evil forces. Watch as he takes on powerful enemies with cutting-edge technology and wit!',
    subscriber: '25.2M Subscribers',
    isLive: true,
    likes: '1.2M',
    comments: '57K',
    shares: '35K',
    friends: [
      {
        name: 'John Doe',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/11192830/media/7690704fa8f0566d572a085637dd1eee.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Jane Smith',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13130602/media/592ccac0a949b39f058a297fd1faa38e.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Alex Johnson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/9165292/media/ccbfbce040e1941972dbc6a378c35e98.jpg?compress=1&resize=1200x1200',
      },
    ],
  },
  {
    id: 'V_ID_2',
    title: 'Captain America',
    thumbnailUrl:
      'https://ew.com/thmb/n2Up7p2IkB-OEWfEq-cxyaB9YOw=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/evans-2000-d2f2889582f445f1a73065966709deb2.jpg',
    duration: '12:18',
    uploadTime: 'May 9, 2011',
    views: '24.9M',
    author: 'Blender Inc.',
    videoUrl:
      'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ElephantsDream.mp4',
    audioUrl: 'http://webaudioapi.com/samples/audio-tag/chrono.mp3',
    description:
      'Join Steve Rogers as he transforms from a skinny soldier to the ultimate super-soldier, Captain America. Witness his heroic journey against the forces of Hydra!',
    subscriber: '25.2M Subscribers',
    isLive: true,
    likes: '935K',
    comments: '48K',
    shares: '30K',
    friends: [
      {
        name: 'Michael Davis',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/7003560/media/48d5ac3503d204751a2890ba82cc42ad.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Sophia Wilson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/6727912/samji_illustrator.jpeg?compress=1&resize=1200x1200',
      },
      {
        name: 'Daniel Martinez',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13661330/media/1d9d3cd01504fa3f5ae5016e5ec3a313.jpg?compress=1&resize=1200x1200',
      },
    ],
  },
  {
    id: 'V_ID_3',
    title: 'Thor',
    thumbnailUrl:
      'https://ew.com/thmb/DeGqRhiBc1U0NjqvFf1iSpEUsTM=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/thor-2000-0d729c17aaca47668a7659a583db6752.jpg',
    duration: '8:18',
    uploadTime: 'May 9, 2011',
    views: '24.9M',
    author: 'T-Series Regional',
    videoUrl:
      'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4',
    description:
      "The mighty Thor, armed with his enchanted hammer Mjolnir, battles to prove his worth as Asgard's hero. Can he reclaim his rightful place as a god?",
    subscriber: '25.2M Subscribers',
    isLive: true,
    likes: '845K',
    comments: '40K',
    shares: '25K',

    friends: [
      {
        name: 'John Doe',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/11192830/media/7690704fa8f0566d572a085637dd1eee.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Jane Smith',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13130602/media/592ccac0a949b39f058a297fd1faa38e.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Alex Johnson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/9165292/media/ccbfbce040e1941972dbc6a378c35e98.jpg?compress=1&resize=1200x1200',
      },
    ],
  },
  {
    id: 'V_ID_4',
    title: 'Doctor Strange',
    thumbnailUrl:
      'https://ew.com/thmb/lOAuVzIS_ozAGDLMmdIsxiusDUs=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/strange-2000-8f262387f0174780989bc42b4afca85a.jpg',
    duration: '8:18',
    uploadTime: 'May 9, 2011',
    views: '24.9M',
    author: 'T-Series Regional',
    videoUrl:
      'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerEscapes.mp4',
    description:
      'Experience the mystical journey of Doctor Strange as he learns the secrets of magic and battles dark forces to protect the multiverse!',
    subscriber: '25.2M Subscribers',
    isLive: false,
    likes: '679K',
    comments: '26K',
    shares: '18K',
    friends: [
      {
        name: 'Michael Davis',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/7003560/media/48d5ac3503d204751a2890ba82cc42ad.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Sophia Wilson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/6727912/samji_illustrator.jpeg?compress=1&resize=1200x1200',
      },
      {
        name: 'Daniel Martinez',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13661330/media/1d9d3cd01504fa3f5ae5016e5ec3a313.jpg?compress=1&resize=1200x1200',
      },
    ],
  },
  {
    id: 'V_ID_5',
    title: 'Spider-Man',
    thumbnailUrl:
      'https://ew.com/thmb/Oek4tZSQFS82lV8iACtVe1q7yCk=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/spiderman-2000-4e34f183efbd40e6aa7a6e2b2725e69e.jpg',
    duration: '8:18',
    uploadTime: 'May 9, 2011',
    views: '24.9M',
    author: 'Vlc Media Player',
    videoUrl:
      'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4',
    description:
      'Peter Parker balances life as a high school student and a superhero. Watch how he swings through the city, fighting crime and saving lives!',
    subscriber: '25.2M Subscribers',
    isLive: true,
    likes: '1.1M',
    comments: '52K',
    shares: '32K',
    friends: [
      {
        name: 'John Doe',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/11192830/media/7690704fa8f0566d572a085637dd1eee.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Jane Smith',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13130602/media/592ccac0a949b39f058a297fd1faa38e.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Alex Johnson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/9165292/media/ccbfbce040e1941972dbc6a378c35e98.jpg?compress=1&resize=1200x1200',
      },
    ],
  },
  {
    id: 'V_ID_6',
    title: 'Hulk',
    thumbnailUrl:
      'https://ew.com/thmb/CLn48jGMiHyaFgCBQ3nJpM04wck=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/hulk-2000-9f0ecd0b6a5140b89ec59b2ab1379f8a.jpg',
    duration: '8:18',
    uploadTime: 'May 9, 2011',
    views: '24.9M',
    author: 'T-Series Regional',
    videoUrl:
      'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4',
    description:
      'Bruce Banner transforms into the Hulk, an unstoppable force of destruction! Witness his rage and power as he fights to control the beast within.',
    subscriber: '25.2M Subscribers',
    isLive: false,
    likes: '790K',
    comments: '33K',
    shares: '22K',
    friends: [
      {
        name: 'Michael Davis',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/7003560/media/48d5ac3503d204751a2890ba82cc42ad.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Sophia Wilson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/6727912/samji_illustrator.jpeg?compress=1&resize=1200x1200',
      },
      {
        name: 'Daniel Martinez',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13661330/media/1d9d3cd01504fa3f5ae5016e5ec3a313.jpg?compress=1&resize=1200x1200',
      },
    ],
  },
  {
    id: 'V_ID_7',
    title: 'Scarlet Witch',
    thumbnailUrl:
      'https://ew.com/thmb/t5o4scgyj-1vT0nceaPIFQm4jKk=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/scarlet-witch-2000-40967f18854642ef87f9b8a10d1d5856.jpg',
    duration: '8:18',
    uploadTime: 'May 9, 2011',
    views: '24.9M',
    author: 'T-Series Regional',
    videoUrl:
      'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerEscapes.mp4',
    description:
      'Wanda Maximoff, the Scarlet Witch, wields incredible reality-altering powers. See how she controls chaos magic and bends reality to her will!',
    subscriber: '25.2M Subscribers',
    isLive: true,
    likes: '923K',
    comments: '42K',
    shares: '28K',
    friends: [
      {
        name: 'John Doe',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/11192830/media/7690704fa8f0566d572a085637dd1eee.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Jane Smith',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13130602/media/592ccac0a949b39f058a297fd1faa38e.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Alex Johnson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/9165292/media/ccbfbce040e1941972dbc6a378c35e98.jpg?compress=1&resize=1200x1200',
      },
    ],
  },
  {
    id: 'V_ID_8',
    title: 'Black Panther',
    thumbnailUrl:
      'https://ew.com/thmb/aRaON4YnrGHgmthcFdm_ULphVyU=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/black-panther-2000-417ccf1a499a4dcfaddab5f6aade5c88.jpg',
    duration: '12:18',
    uploadTime: 'May 9, 2011',
    views: '24.9M',
    author: 'Marvel Studios',
    videoUrl:
      'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/TearsOfSteel.mp4',
    description:
      'Step into Wakanda and witness the rise of T’Challa as he takes on the mantle of the Black Panther, defending his nation from powerful foes.',
    subscriber: '25.2M Subscribers',
    isLive: false,
    likes: '1.3M',
    comments: '68K',
    shares: '46K',
    friends: [
      {
        name: 'Michael Davis',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/7003560/media/48d5ac3503d204751a2890ba82cc42ad.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Sophia Wilson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/6727912/samji_illustrator.jpeg?compress=1&resize=1200x1200',
      },
      {
        name: 'Daniel Martinez',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13661330/media/1d9d3cd01504fa3f5ae5016e5ec3a313.jpg?compress=1&resize=1200x1200',
      },
    ],
  },

  {
    id: 'V_ID_9',
    title: 'ShadowHunter99',
    thumbnailUrl:
      'https://ew.com/thmb/aRaON4YnrGHgmthcFdm_ULphVyU=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/black-panther-2000-417ccf1a499a4dcfaddab5f6aade5c88.jpg',
    duration: '12:18',
    uploadTime: 'May 9, 2011',
    views: '24,969,123',
    author: 'Blender Inc.',
    videoUrl:
      'https://raw.githubusercontent.com/kartikeyvaish/React-Native-UI-Components/main/src/Reels/config/videos/sample.mp4',
    description:
      'Step into Wakanda and witness the rise of T’Challa as he takes on the mantle of the Black Panther, defending his nation from powerful foes.',
    subscriber: '25.2M Subscribers',
    isLive: false,
    likes: '1.3M',
    comments: '68K',
    shares: '46K',
    category: 'Action',
    friends: [
      {
        name: 'John Doe',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/11192830/media/7690704fa8f0566d572a085637dd1eee.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Jane Smith',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13130602/media/592ccac0a949b39f058a297fd1faa38e.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Alex Johnson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/9165292/media/ccbfbce040e1941972dbc6a378c35e98.jpg?compress=1&resize=1200x1200',
      },
    ],
  },
  {
    id: 'V_ID_10',
    title: 'EvyTheExplorer',
    thumbnailUrl:
      'https://ew.com/thmb/aRaON4YnrGHgmthcFdm_ULphVyU=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/black-panther-2000-417ccf1a499a4dcfaddab5f6aade5c88.jpg',
    duration: '10:45',
    uploadTime: 'June 15, 2012',
    views: '18,765,432',
    author: 'Marvel Studios',
    videoUrl:
      'https://raw.githubusercontent.com/kartikeyvaish/React-Native-UI-Components/main/src/Reels/config/videos/sampleLandscape.mp4',
    description:
      'Experience the journey of T’Challa as he embraces his destiny, protecting Wakanda from threats both within and beyond.',
    subscriber: '30.1M Subscribers',
    isLive: false,
    likes: '980K',
    comments: '54K',
    shares: '38K',
    category: 'Sci-Fi',
    friends: [
      {
        name: 'Michael Davis',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/7003560/media/48d5ac3503d204751a2890ba82cc42ad.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Sophia Wilson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/6727912/samji_illustrator.jpeg?compress=1&resize=1200x1200',
      },
      {
        name: 'Daniel Martinez',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13661330/media/1d9d3cd01504fa3f5ae5016e5ec3a313.jpg?compress=1&resize=1200x1200',
      },
    ],
  },
  {
    id: 'V_ID_11',
    title: 'WarriorX23',
    thumbnailUrl:
      'https://ew.com/thmb/aRaON4YnrGHgmthcFdm_ULphVyU=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/black-panther-2000-417ccf1a499a4dcfaddab5f6aade5c88.jpg',
    duration: '15:30',
    uploadTime: 'March 22, 2015',
    views: '32,543,987',
    author: 'Cinematic Universe',
    videoUrl:
      'https://raw.githubusercontent.com/kartikeyvaish/React-Native-UI-Components/main/src/Reels/config/videos/samplePortrait.mp4',
    description:
      'Follow T’Challa’s transformation from prince to Black Panther as he battles for his people’s future.',
    subscriber: '27.5M Subscribers',
    isLive: false,
    likes: '2.1M',
    comments: '95K',
    shares: '52K',
    category: 'Adventure',
    friends: [
      {
        name: 'Michael Davis',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/7003560/media/48d5ac3503d204751a2890ba82cc42ad.jpg?compress=1&resize=1200x1200',
      },
      {
        name: 'Sophia Wilson',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/6727912/samji_illustrator.jpeg?compress=1&resize=1200x1200',
      },
      {
        name: 'Daniel Martinez',
        imageUrl:
          'https://cdn.dribbble.com/users/3281732/screenshots/13661330/media/1d9d3cd01504fa3f5ae5016e5ec3a313.jpg?compress=1&resize=1200x1200',
      },
    ],
  },
];
